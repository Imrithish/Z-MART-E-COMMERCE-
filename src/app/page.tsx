...
export default function Home() {
  ...
  const curatedSections = useMemo(() => {
    ...
    return {
      newArrivals: products.slice(0, 5),
      bestSellers: products.filter((p: any) => p.rating >= 4.8 || p.isBestSeller).slice(0, 5),
      popular: products.filter((p: any) => p.reviews > 1000).slice(0, 5)
    };
  }, [products]);
  ...
  const ProductCard = ({ product }: { product: any }) => (
    <div 
      className="group cursor-pointer flex flex-col h-full bg-white p-2.5 rounded-2xl shadow-[0_4px_12px_-6px_rgba(0,0,0,0.05)] hover:shadow-[0_15px_30px_-10px_rgba(0,0,0,0.1)] transition-all duration-500 ease-out border border-slate-100/50 hover:border-primary/20 hover:-translate-y-1"
      onClick={() => handleProductClick(product)}
    >
      <div className="relative aspect-[4/3] bg-slate-50/50 rounded-xl overflow-hidden p-2 mb-2 group-hover:bg-white transition-colors duration-500">
        <Image 
        ...
        />
        {product.isDeal && (
          <Badge className="absolute top-1.5 left-1.5 bg-red-600 text-white border-none text-[6px] font-black tracking-widest px-1 py-0.5 rounded shadow-lg">
            DEAL
          </Badge>
        )}
      </div>
      ...
      <div className="flex flex-col flex-1 space-y-1">
        <div className="space-y-0.5">
           <p className="text-[6px] font-black text-primary uppercase tracking-[0.2em] mb-0.5">{product.category}</p>
           <h3 className="text-[10px] font-extrabold text-slate-800 line-clamp-1 uppercase tracking-tight leading-tight group-hover:text-primary transition-colors">
             {product.name}
           </h3>
        ...
        <div className="mt-auto pt-1 space-y-1.5">
          <div className="flex items-center justify-between">
            <div className="flex flex-col">
              <span className="text-xs font-black text-slate-900 tracking-tighter">{formatCurrency(product.price)}</span>
            </div>
            <button 
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="h-7 w-7 flex items-center justify-center rounded-lg bg-slate-50 border border-slate-100 text-slate-400 hover:text-white hover:bg-slate-900 hover:border-slate-900 transition-all duration-300 shadow-sm"
            >
          ...
          <Button 
            ...
            className="w-full h-7 bg-slate-900 hover:bg-primary text-white hover:text-slate-900 font-black uppercase tracking-widest text-[7px] rounded-lg transition-all duration-300 shadow-lg shadow-slate-900/5 active:scale-95 border-none"
          >
            <Zap className="h-2.5 w-2.5 mr-1 fill-current" /> Buy Now
          </Button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-body">
      ...
      <main className="flex-1 pb-24">
        ...
        <div className="max-w-[1450px] mx-auto px-4 md:px-8 py-16 space-y-16">
              ...
              <div className="flex items-end justify-between border-b border-slate-200 pb-2">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">New Arrivals</h2>
                  </div>
                </div>
              </div>
          ...
          {categoryEntries[0] && (
            <section className="space-y-4">
              <div className="flex items-end justify-between border-b border-slate-200 pb-2">
                <div className="space-y-0.5">
                  <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">{categoryEntries[0][0]}</h2>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                {categoryEntries[0][1].slice(0, 5).map((product) => (
              ...
              <div className="flex items-end justify-between border-b border-slate-200 pb-2">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Best Sellers</h2>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          ...
          {categoryEntries[1] && (
            <section className="space-y-4">
              <div className="flex items-end justify-between border-b border-slate-200 pb-2">
                <div className="space-y-0.5">
                  <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">{categoryEntries[1][0]}</h2>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                {categoryEntries[1][1].slice(0, 5).map((product) => (
                  <ProductCard key={product.id} product={product} />
                ))}
              </div>
            </section>
              ...
              <div className="flex items-end justify-between border-b border-slate-200 pb-2">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Popular Now</h2>
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5 gap-3">
